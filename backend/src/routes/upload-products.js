...
router.post('/api/upload/product-data', async (req, res) => {
    try {
        ...
        for (const productData of products) {
            try {
                const product = await (0, supabaseClient_1.createProduct)(productData);
                // Generate embeddings for product description
                if (productData.description) {
                    const embedding = await (0, openaiClient_1.getEmbedding)(productData.description);
    ...
    try {
        ...
        for (const file of files) {
            try {
                ...
                const publicUrl = await (0, supabaseClient_1.uploadFile)('product-images', filePath, fileBuffer);
    ...
    try {
        ...
        if (Array.isArray(data)) {
            ...
            for (const product of data) {
                const created = await (0, supabaseClient_1.createProduct)(product);
            ...
            // Single product
            const created = await (0, supabaseClient_1.createProduct)(data);
            return {
                type: 'json',
...
async function processImageFile(filePath, originalName) {
    ...
    const publicUrl = await (0, supabaseClient_1.uploadFile)('product-images', fileName, fileBuffer);
    return {
        type: 'image',
...
router.get('/api/upload/status', async (req, res) => {
    try {
        const { getEmbeddingsStats } = await import('../lib/retrieval');
        const { getProducts } = await import('../lib/supabaseClient');
...
//# sourceMappingURL=upload-products.js.map