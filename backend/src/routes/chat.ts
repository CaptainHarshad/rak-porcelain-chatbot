...
router.post('/api/chat/stream', async (req, res) => {
  try {
    ...
    // Stream the response
    const { chatCompletionStream } = await import('../lib/openaiClient.js');
...
router.get('/api/chat/history/:sessionId', async (req, res) => {
  try {
    ...
    const { getConversationHistory } = await import('../lib/supabaseClient.js');
...
router.get('/api/chat/health', async (req, res) => {
  try {
    const { validateRetrieval } = await import('../lib/retrieval.js');
    const { validateApiKey } = await import('../lib/openaiClient.js');
